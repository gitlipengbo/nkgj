...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 185,292);
        ctx.fillText(time1, 365, 292);
        ctx.fillText(sj.zjs+"局", 560, 292);
        ...
        ctx.fillText(writeContent,writeLeft,352);

        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ctx.fillStyle = "#19170b";
                ctx.fillRect(140,529-(0-(i-2)*76),517,68);
            }
            var user=sj['user'][i];
            ctx.font = "37px bolder songti";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 195, 380+(i*75));
            ctx.fillText(decode64(user.nickname).substring(0,9), 219, 426+(i*75));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,9), 219, 426+(i*75));
                ctx.fillText(user.dqjf, 532, 426+(i*75));
            }
            ctx.fillText( user.dqjf, 532, 426+(i*75));
        }
