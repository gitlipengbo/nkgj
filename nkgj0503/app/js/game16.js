...
function overroom(data){
   ...
   overzt=0;
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 178,232);
        ctx.fillText(time1, 361, 232);
        ctx.fillText(sj.zjs+"局", 580, 232);
        ...
        ctx.fillText(writeContent,writeLeft,290);

        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ctx.fillStyle = "#15102e";
                ctx.fillRect(142,480-(0-(i-2)*80),525,73);
            }
            var user=sj['user'][i];
            ctx.font = "31px bolder songti";
            ctx.fillStyle = "#d8cb66";
            ctx.fillText(decode64(user.nickname).substring(0, 9), 240, 370+i*78);
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0, 9), 240, 370+i*78);
                ctx.fillText(user.dqjf, 545, 370-(0-i*78));
            }
            ctx.fillText(user.dqjf, 545, 370-(0-i*78));
        }
