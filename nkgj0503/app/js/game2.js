...
function overroom(data){
    ...
    img.onload = function(){
        console.log(sj);
        ctx.drawImage(img,0,0,802,1297);
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(142,519-(0-(i-2)*75),521,71);
            }
            var user=sj['user'][i];
            ctx.font = "32px bolder songti";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 195, 380+(i*75));
                ctx.fillText(decode64(user.nickname).substring(0,10), 219, 423+(i*75));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,10), 219, 423+(i*75));
                ctx.fillText(user.dqjf, 543, 423+(i*75));
            }
            ctx.fillText( user.dqjf, 543, 423+(i*75));
        }
